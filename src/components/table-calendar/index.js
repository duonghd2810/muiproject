        ...
        {Object.values(MOMENT)
          ...
          .map((period) => {
            const moment = Object.entries(MOMENT).reduce(
              (val, [key, value]) => (value.includes(period) ? key : val),
              ""
            );
            const rowSpan =
              MOMENT[moment][0] === period ? MOMENT[moment].length : 0;

            return (
              <tr>
                {!!rowSpan && <td rowSpan={rowSpan}>{moment}</td>}
                <td>{period}</td>
                {WEEK.map((date) => {
                  const subject = (map[date] || []).find(({ lesson }) =>
                    lesson.split(",").includes(period.toString())
                  );
                  if (subject) {
                    const { lesson, tenhp, id_classroom, teacherName } =
                      subject;
                    if (lesson.startsWith(period)) {
                      return (
                        <td
                          rowSpan={lesson.split(",").length}
                          style={{ background: "antiquewhite" }}
                        >
                          {tenhp} <br />
                          {teacherName} <br />({id_classroom})<br />
                        </td>
                      );
                    }
                    return null;
